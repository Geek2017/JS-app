    ...
    for (var key in sessionStorage) {
        ...
        if (sessionStorage[key] != 0) {
            if (key <= lt) {
                ...
                coins += '<td width="40%" class="text-right">' + (key * sessionStorage[key]).toFixed(2) + ' ' + $("#symb").text() + '</td></tr></div>';
                ...
                banks += '<td width="40%" class="text-right">' + (key * sessionStorage[key]).toFixed(2) + ' ' + $("#symb").text() + '</td></div></tr>';
                notes = notes * 1 + (key * sessionStorage[key]);
                localStorage.setItem('2Coin', notes + ' ' + value);
...
$("#sendemail").click(function(e) {
 if(sessionStorage.length == 0){
    ...
    $.ajax({
        type: 'GET',
        url: 'https://www.wylog.com/ingenico/ws/weather/index.php/emailsend?authID=ce0cf133c8e40246ec816cef2375ca4b&lang=fr',
        data: {
